...
var dat = ee.FeatureCollection("users/eeProject/GROD/GROD_dupesRemoved_20191028"),
...
Map.addLayer(dat, {color: 'yellow'}, 'All structures in "Uncertain"');

var groupId = dat.aggregate_array('group_id').getInfo();

var instructions = 'Index range: [0, 2939]; \nSayali: [1, 1469]\nMike: [1470, 2939]';
Map.add(ui.Label(instructions));

var layers = Map.layers();

var mapPointByIndex = function(i) {
  var thisGroup = ee.FeatureCollection(dat.toList(1, i));//filterMetadata('group_id', 'equals', i);
...
Export.table.toDrive({
  collection: merged,
  description: NAME + '_replace_uncertain',
  folder: '',
  fileNamePrefix: NAME + '_replace_uncertain',
  fileFormat: 'csv'});
Map.add(controls);
